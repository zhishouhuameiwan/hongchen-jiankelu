...
function MapPage() {
  ...
  const store = useGameStore()
  return <main><TopBar /><nav><button onClick={() => store.go('heroine')}>红颜</button><button onClick={() => store.go('deck')}>卡组</button><button onClick={store.advancePhase}>结束{state.phase === 'day' ? '白天' : '夜晚'}</button></nav><GoalPanel state={state} /><section className="grid">{locations.map((loc) => { const selected = pickEventForLocation(state, events, loc.id); const affordable = state.stamina >= loc.staminaCost; return <button className="card" key={loc.id} disabled={!affordable} onClick={() => store.exploreLocation(loc.id)}><h3>{loc.name}</h3><p>{state.phase === 'day' ? loc.dayDescription : loc.nightDescription}</p><small>路程体力 -{loc.staminaCost}</small><small>{selected ? `可触发：${selected.title}` : '暂无特殊事件'}</small>{!affordable ? <small>体力不足，先结束时段休整</small> : null}</button> })}</section><Log /></main>
}
